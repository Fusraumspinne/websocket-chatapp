...
export async function POST(req) {
    try {
        const { id, message, userName, roomName, timestamp, edited, response } = await req.json();
        await connectMongoDB()
        await Message.create({ id, message, userName, roomName, timestamp, edited, response });
